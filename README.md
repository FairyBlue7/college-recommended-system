...
⚠️ **警告**：本项目包含故意设计的安全漏洞，仅用于教学目的。

### 功能特性

本系统包含一个完整的 SQL 注入安全演示模块，用于安全教育和安全意识培训：

- **漏洞演示登录页面**：故意存在 SQL 注入漏洞的登录功能
- **攻击场景模拟**：演示 5 种常见的 SQL 注入攻击方式
- **安全代码对比**：并排展示漏洞代码和安全代码的差异
- **交互式教学页面**：实时显示 SQL 查询构建过程
- **CTF 夺旗挑战**：通过 SQL 注入找到隐藏的 flag

### 使用方法

1. **访问安全教育页面**：
   ```
   http://localhost:5000/security_demo
   ```
   查看 SQL 注入原理、防护措施和交互式演示

2. **尝试攻击漏洞页面**：
   ```
   http://localhost:5000/vulnerable_login
   ```
   使用提供的 Payload 尝试 SQL 注入攻击

3. **阅读详细文档**：
   ```
   查看 SECURITY_DEMO.md 文件
   ```
   了解详细的攻击步骤和防护方法
...
**场景 2：联合查询注入**
- 用户名: `' UNION SELECT id, username, email, password_hash, role FROM users --`

**场景 3：CTF 挑战 - 寻找隐藏的 Flag**
- 用户名: `' UNION SELECT id, flag_name, flag_value, hint, 'user' FROM hidden_flags --`

### 教学要点

1. **理解威胁**：通过实际操作理解 SQL 注入的危害
2. **对比学习**：通过漏洞代码和安全代码的对比学习正确的编码方式
3. **防护实践**：学习参数化查询等安全编码实践
4. **安全意识**：培养在开发过程中的安全意识
